            ...
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        <div>
                            <h5 class="mb-0">{{ __('Danh sách khảo sát') }}</h5>
                        </div>
                        <a href="{{ route('survey.create') }}" class="btn bg-gradient-primary btn-sm mb-0 px-2"
                            type="button"><i class="fa-solid fa-plus"></i> {{ __('Tạo khảo sát') }}</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($blogs as $key => $post)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('survey.edit', $post->id) }}" class="mx-3"
                                                title="{{ __('edit') }}">
                                                <i class="fa-solid fa-pencil"></i>
                                            </a>
                                            @include('admin.pages.components.delete-form', [
                                                'id' => $post->id,
                                                'route' => route('survey.destroy', $post->id),
