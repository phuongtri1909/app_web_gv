        ...
        <div class="col-12">
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <h5 class="mb-0">{{ __('Tạo hội chợ') }}</h5>
                </div>
                <div class="card-body  pt-4 p-3">
                    @include('admin.pages.notification.success-error')
                    <form action="{{ route('fair-registrations.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            @foreach ($languages as $language)
                                ...
                                <div class="form-group mb-3 col-12">
                                    ...
                                    <textarea name="content_{{ $language->locale }}" id="content_{{ $language->locale }}"
                                        class="form-control my-editor @error('content_'.$language->locale) is-invalid @enderror" rows="3">{{ old('content_' . $language->locale) }}</textarea>
                        ...
                        <a href="{{ route('fair-registrations.index') }}"
