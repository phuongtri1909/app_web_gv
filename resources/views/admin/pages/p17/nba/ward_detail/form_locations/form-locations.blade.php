...
@section('title', 'Địa điểm khu phố')
...
@section('content-auth')
    <section id="start-promotion">
        <div class="container my-4">
            <div class="row">
                <form action="{{ route('form.location.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="districts_govap_id" class="form-label">Khu phố <span
                                    class="text-danger">*</span></label>
                            <select id="districts_govap_id" name="districts_govap_id"
                                class="form-select form-select-sm @error('districts_govap_id') is-invalid @enderror" required>
                                <option value="" disabled selected>Chọn khu phố</option>
                                @foreach ($district as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('districts_govap_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('districts_govap_id')
                                <div class="invalid-feedback" role="alert">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="text-center">
                        ...
                        <button type="submit" class="btn bg-app-gv rounded-pill text-white mt-3">Thêm địa điểm</button>
                    </div>
                </form>
            </div>
        </div>
