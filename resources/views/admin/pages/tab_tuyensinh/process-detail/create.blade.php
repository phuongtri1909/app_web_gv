    ...
    <div class="col-12">
        <div class="card mb-4 mx-4">
            <div class="card-header pb-0 px-3">
                <h5 class="mb-0">{{ __('add_detail_process') }}</h5>
            </div>
            <div class="card-body pt-4 p-3">
                
                @include('admin.pages.notification.success-error')

                <form action="{{ route('admission-process-detail.store',$process->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        @foreach ($languages as $language)
                            <div class="form-group mb-3 col-12">
                                ...
                                <textarea name="content_{{ $language->locale }}" id="content_{{ $language->locale }}"
                                    class="form-control @error('content_{{ $language->locale }}') is-invalid @enderror">{!! old('content_'.$language->locale) !!}</textarea>
