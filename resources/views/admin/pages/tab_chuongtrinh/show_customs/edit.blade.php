    ...
    <div class="col-12">
        <div class="card mb-4 mx-4">
            <div class="card-header pb-0 px-3">
                <h5 class="mb-0">{{ __('edit_tab_custom') }}</h5>
            </div>
            <div class="card-body pt-4 p-3">

                @include('admin.pages.notification.success-error')

                <form action="{{ route('tabs-customs.update', $tabs_customs->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        @foreach ($languages as $language)
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="text" name="{{ 'title_'.$language->locale }}"
                                    ...
                                    value="{{ old('title_'.$language->locale, $tabs_customs->getTranslation('title', $language->locale)) }}" required>
                            ...
                            <div class="form-group mb-3 col-md-6">
                                <label for="{{ 'description_'.$language->locale }}">{{ __('description') }}:
                                    {{ $language->name }}</label>
                                <input type="text" name="{{ 'description_'.$language->locale }}"
                                    id="{{ 'description_'.$language->locale }}"
                                    class="form-control @error('description_'.$language->locale) is-invalid @enderror"
                                    value="{{ old('description_'.$language->locale, $tabs_customs->getTranslation('description', $language->locale)) }}" required>
                            ...
                            <div class="form-group mb-3 col-md-12">
                                <label for="{{ 'content_'.$language->locale }}">{{ __('content') }}:
                                    {{ $language->name }}</label>
                                <textarea name="{{ 'content_'.$language->locale }}" id="{{ 'content_'.$language->locale }}"
                                    ...
                                    rows="4">{{ old('content_'.$language->locale, $tabs_customs->getTranslation('content', $language->locale)) }}</textarea>
                        ...
                        <div class="text-center">
                            ...
                            <a href="{{ route('tabs-programs.show',$tabs_customs->tab_id) }}" class="btn btn-secondary">{{ __('cancel') }}</a>
                        </div>
                    </div>
                </form>
