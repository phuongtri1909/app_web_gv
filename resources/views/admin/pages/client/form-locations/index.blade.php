                            ...
                            @else
                                @foreach ($locations as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <p class="text-xs font-weight-bold mb-0 text-truncate-2">{{ $item->name ?? '-' }}</p>
                                        </td>

                                        <td>
                                            <p class="text-xs font-weight-bold mb-0">{{ $item->address_address ?? '-' }}</p>
                                        </td>

                                        <td>
                                            <p class="text-xs font-weight-bold mb-0">{{ $item->businessField->name ?? '-' }}</p>
                                        </td>
                                        ...
                                        <td class="text-center">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <a class="dropdown-item" href="#"
                                                                onclick="updateStatus('Locations', {{ $item->id }}, 'approved')">
                                                                <i class="fas fa-check-circle text-success me-2"></i>Duyệt
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item" href="#"
                                                                onclick="updateStatus('Locations', {{ $item->id }}, 'rejected')">
                                                                <i class="fas fa-times-circle text-danger me-2"></i>Từ chối
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item" href="#"
                                                                onclick="updateStatus('Locations', {{ $item->id }}, 'pending')">
                                            ...
                                            @include('admin.pages.components.delete-form', [
                                                'id' => $item->id,
                                                'route' => route('locations.destroy', $item->id),
                                                'message' => 'Bạn có chắc chắn muốn xóa điểm đến này không?',
                                            ])
                                            <a href="{{ route('locations.show',$item->id) }}" class="mx-3 view-business" data-id="{{ $item->id }}" title="{{ __('Xem chi tiết') }}">
