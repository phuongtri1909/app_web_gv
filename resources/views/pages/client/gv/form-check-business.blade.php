    ...
    <div class="container">
        <form action="{{ route('form.check.business') }}" method="post">
            ...
            <div class="form-group">
                ...
                <input value="{{ old('business_code') }}" placeholder="Mã doanh nghiệp" type="text" name="business_code" id="business_code" class="form-control form-control-sm" required>
                @if ($errors->has('business_code'))
                    <span class="text-danger">{{ $errors->first('business_code') }}</span>
                @endif
            </div>
            <div class="text-center">
                <button type="submit" class="btn bg-app-gv rounded-pill text-white m-2">Kiểm tra</button>
            </div>
        </form>
    </div>
@endsection