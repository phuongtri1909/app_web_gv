    ...
    <div class="col-12">
        <div class="card mb-4 mx-4">
            <div class="card-header pb-0 px-3">
                <h5 class="mb-0">{{ __('edit_content_component1_tab', ['tab'=> $tab->title]) }}</h5>
            </div>
            <div class="card-body pt-4 p-3">
                
                @include('admin.pages.notification.success-error')

                <form action="{{ route('content_one_tab.update',$img_content->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        @foreach ($languages as $language)
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="text" name="{{ 'title_'.$language->locale }}"
                                    ...
                                    value="{{ old('title_'.$language->locale,$img_content->getTranslation('title',$language->locale))}}" required>
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <textarea name="content_{{ $language->locale }}" id="content_{{ $language->locale }}"
                                    ...
                                    required>{{ old('content_'.$language->locale,$img_content->getTranslation('content',$language->locale)) }}</textarea>
